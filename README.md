...
│                                               │
│  Web 管理服务 :18080                           │
│    └── Supervisor 私有套接字控制 sdfxd 启停     │
│                                               │
│  持久化卷 /var/lib/sdfx                        │
└───────────────────────────────────────────────┘
```

项目不提供面向应用的独立 Unix 密码管理接口。Web 后端复用 18081 服务发送
密钥、设备和会话管理命令，并使用容器启动时生成、仅保存在
`/run/sdfx/admin.token` 的随机令牌鉴权。进程启停使用仅限容器内 `sdfx`
用户访问的 `/run/sdfx/supervisor.sock`，不会对外开放新的 TCP 端口。
...
- 设备状态、活动会话和累计请求；
- 服务管理展示 SDF TCP 监听地址、端口、Supervisor 状态、启动/停止时间、运行时长、PID、请求数、会话数和密钥数；
- 系统管理员可从 Web 真正启动、停止或重启同一容器内的 `sdfxd`；Web 本身保持在线，操作要求确认文字并写入审计；
- 设备信息修改；
...
- SM2、RSA 和对称密钥可配置有效期；Web 显示创建时间、到期时间、30 天内到期告警和已到期告警。教学模式下到期仅告警，不自动停用密钥；
- SDF 会话查看和强制终止；
