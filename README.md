...
本机访问 `http://127.0.0.1:18080`，局域网电脑访问 `http://服务器IP:18080`，然后完成首次初始化。
...
（SPDX：`AGPL-3.0-only`）发布，版权人为 `sawanolin and CryptoKit SoftHSM
contributors`。通过网络向用户提供修改版服务时，必须依 AGPL 第 13 条向
