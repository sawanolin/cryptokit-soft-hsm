...
int main(void)
{
    ...
    CHECK_OK(SDF_ExportSignPublicKey_ECC(session, 1, &sign_public));
    CHECK_OK(SDF_ExportEncPublicKey_ECC(session, 1, &enc_public));
    if (memcmp(&sign_public, &enc_public, sizeof(sign_public)) == 0) {
        fprintf(stderr, "signing and encryption keys must be distinct\n");
        goto cleanup;
    }

    LONG ret = SDF_InternalSign_ECC(session, 1, digest, sizeof(digest), &signature);
    if (ret != SDR_PARDENY) {
        fprintf(stderr, "sign without access returned 0x%08x\n", (unsigned int)ret);
        goto cleanup;
    }
    ret = SDF_GetPrivateKeyAccessRight(session, 1, "definitely-wrong", 16);
    if (ret != SDR_PRKRERR) {
        fprintf(stderr, "wrong password returned 0x%08x\n", (unsigned int)ret);
        goto cleanup;
    }

    CHECK_OK(SDF_GetPrivateKeyAccessRight(session, 1, (LPSTR)password,
                                          (ULONG)strlen(password)));
    CHECK_OK(SDF_InternalSign_ECC(session, 1, digest, sizeof(digest), &signature));
    CHECK_OK(SDF_InternalVerify_ECC(session, 1, digest, sizeof(digest), &signature));
    digest[0] ^= 1;
    ret = SDF_InternalVerify_ECC(session, 1, digest, sizeof(digest), &signature);
    digest[0] ^= 1;
    ...
    CHECK_OK(SDF_GenerateKeyWithIPK_ECC(session, 1, 128, wrapped,
                                        &generated_key));
    CHECK_OK(SDF_ImportKeyWithISK_ECC(session, 1, wrapped, &imported_key));
    ...
    CHECK_OK(SDF_ReleasePrivateKeyAccessRight(session, 1));
    ret = SDF_InternalSign_ECC(session, 1, digest, sizeof(digest), &signature);
    if (ret != SDR_PARDENY) {
        fprintf(stderr, "sign after release returned 0x%08x\n", (unsigned int)ret);
        goto cleanup;
    }
    ret = SDF_ImportKeyWithISK_ECC(session, 1, wrapped, &imported_key);
    if (ret != SDR_PARDENY) {
